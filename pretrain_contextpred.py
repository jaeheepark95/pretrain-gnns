...
# [0, 1, ... , num-1] 텐서 생성하고, 왼쪽으로 shift 만큼 shift함
# ex) cycle_index(5, 1) => tensor([1, 2, 3, 4, 0])
...
# train 에서는 한번에 gnn모델을 2개 학습시켜야함.
# 1) pre-training을 학습하기 위한 모델 2) context graph embedding을 학습하기 위한 모델
# model_substruct: pretraining model, model_context: context graph model

# args는 argument parser에 있는 변수들 다 넣어주는듯.
...
def main():
    ...
    # context subgraph를 생성할 때, l1-hop ~ l2-hop 까지의 graph를 생성함.
    # l1 은 GNN의 message passing num_layer보다 1 작게 설정함.
    # csize : context size
    l1 = args.num_layer - 1
    l2 = l1 + args.csize
